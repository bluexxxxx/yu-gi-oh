  ...
  <h1>專欄管理</h1>
    <div class="container">
      ...
      <n-modal v-model:show="showModal" preset="dialog">
        <h2>{{ form._id.length > 0 ? '編輯專欄' : '新增專欄' }} </h2> 
            ...
            <tbody>
              <template v-for='(deck, idx) in sliceDecks'>
                <tr v-if='sliceDecks.length > 0' :key='deck._id'>
                  ...
                  <td v-if="currentPage === 1"><n-button type="info" @click="openDialog(deck._id, idx)" :loading="loading"> 編輯 </n-button></td>
                  <td v-if="currentPage > 1"><n-button type="info" @click="openDialog(deck._id, idx + ((currentPage-1) * pageSize))" :loading="loading"> 編輯 </n-button></td>

                  <td v-if="currentPage === 1"><n-button type="error" @click="del(deck._id, idx)" :loading="loading"> 刪除 </n-button></td>
                  <td v-if="currentPage > 1"><n-button type="error" @click="del(deck._id, idx + ((currentPage-1) * pageSize))" :loading="loading"> 刪除 </n-button></td>
                </tr>
              </template>
            </tbody>
          </n-table>
